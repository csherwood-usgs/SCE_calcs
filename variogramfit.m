...
if params.plotit
    switch lower(type)
        case 'bounded'
            hh=plot(h,gammaexp,'ok','MarkerSize',8,'color',[.3 .3 .3]);
            hold on
            h1=fplot(@(h) funnugget(b) + func(b,h),[0 b(1)]);
            set(h1,'color',[.4 .4 .8],'linewidth',2.5)
            h2=fplot(@(h) funnugget(b) + b(2),[b(1) max(h)]);
            set(h2,'color',[.6 .6 .9],'linewidth',2.5)

            
        case 'unbounded'
            plot(h,gammaexp,'ok','MarkerSize',9);
            hold on
            fplot(@(h) funnugget(b) + func(b,h),[0 max(h)])
    end
    axis([0 max(h) 0 max(gammaexp)])
    xlabel('Lag distance (m)','fontsize',14)
    ylabel('Semivariance  \gamma','fontsize',14)
    hold off
end
